class PatientState < ActiveRecord::Base
  ...
  belongs_to :state, :class_name => 'ProgramWorkflowState', :foreign_key => :state
  belongs_to :patient_state, :class_name => 'PatientState', :foreign_key => :patient_state_id
  has_many :patient_states, :class_name => 'PatientState', :foreign_key => :patient_state_id
